...
Two metric conventions remain stable. In cross-modal retrieval, \texttt{S} denotes the conservative score \texttt{min(A2M, M2A)} for the relevant evaluation pool. In Escalon 2, bootstrap intervals are grouped by speaker; in Escalon 1, the canonical summaries use stabilized reference readings rather than re-deriving the entire raw evaluation tree here. Where a row comes from evaluation seeds over a fixed checkpoint rather than from full training-seed reruns, that status is marked explicitly. Where \texttt{best} and \texttt{final} differ, both are reported explicitly to avoid flattening early peaks into final closures.
...
\texttt{d4a4} & dual guided (\texttt{D4+A4}) & $84.1\% \pm 2.3$ (eval-seed) & $+8.9$ & flagship guided arm; full training-seed replication pending & positive reference \\
...
Mechanism clarification & Gate 10 closes the mechanism contrast in the retrospective branch & Mechanism dominates over descriptor content; concat $>$ FiLM $\gg$ attn bias & closed \\
New sensor-physics front & E2: Speech$\leftrightarrow$EGG with strong baseline, closed null & Extends program outside symbolic music pairing & closed null \\
Encoder-regime comparison & S2-P3 completes first WavLM-Large frozen pass & Shifts live question from mechanism to encoder regime; next step = P2 vs P3 & active \\
