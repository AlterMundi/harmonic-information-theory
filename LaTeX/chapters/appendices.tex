...
This glossary fixes the meaning of the manuscript's main terms as they are used here. It is not a general dictionary of music, neuroscience, or philosophy. It is a compact operational map of the vocabulary stabilized across Chapters 1-15.
...
Two metric conventions remain stable. In cross-modal retrieval, \texttt{S} denotes the conservative score \texttt{min(A2M, M2A)} for the relevant evaluation pool. In Escalon 2, bootstrap intervals are grouped by speaker; in Escalon 1, the canonical summaries use the already stabilized multi-seed or gate-level readings rather than re-deriving the entire raw evaluation tree here. Where \texttt{best} and \texttt{final} differ, both are reported explicitly to avoid flattening early peaks into final closures.
...
\begin{table}[ht]
\centering
\caption{Escalon 1 flagship retrieval summary. Multi-seed closed comparison for the canonical arms used in the main chapter.}
\label{tab:e1-flagship}
\footnotesize
\begin{tabularx}{\textwidth}{l l c c X l}
\toprule
\textbf{Arm} & \textbf{Descriptor route} & \textbf{\texttt{S} (multi-seed)} & \textbf{$\Delta$ vs \texttt{D0}} & \textbf{Reading} & \textbf{Status} \\
\midrule
\texttt{D0} & none & $75.2\% \pm 2.3$ & $0.0$ & unguided shared-space baseline & closed baseline \\
\texttt{d4a4} & dual guided (\texttt{D4+A4}) & $84.1\% \pm 2.3$ & $+8.9$ & flagship guided arm & closed positive \\
...
Test02 & shuffled descriptor & $73.6\%$ & $-9.4$ & control without correct pairing & closed pos. \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
\texttt{D0} & $0.435$ & $0.597$ & $80.4\%$ & baseline geometry and fine discrimination \\
...
\texttt{a4r-pca} & audio-side proj.\ cond. & $82.6\%$ & 30 & $+3.4$ & closed positive \\
...
\begin{table}[ht]
\centering
\caption{Gate 10 mechanism sweep. Final results, 9/9 arms at 30 epochs.}
\label{tab:gate10}
\footnotesize
...
\texttt{a7-pca} & \texttt{a7} & FiLM/\texttt{pca} & 30 & $71.8\%$ & sixth & closed \\
\texttt{a10a-ab} & \texttt{a10a} & attn bias & 30 & $59.6\%$ & seventh & closed \\
\texttt{a10d-ab} & \texttt{a10d} & attn bias & 30 & $57.2\%$ & eighth & closed \\
\texttt{a7-ab} & \texttt{a7} & attn bias & 30 & $55.8\%$ & ninth & closed \\
\bottomrule
\end{tabularx}
\end{table}

Final ranking: \texttt{concat > FiLM/pca} (+2pp) \texttt{>> attn\_bias} (+16pp). Mechanism dominates over descriptor content (2--3pp spread within each mechanism class). All arms closed at comparable epochs.


\section{Escalon 2 - Full null and descriptor-by-mechanism inventory}


The second front produced a real cross-modal baseline and a real descriptor program, but not a positive descriptor lift under the first completed mechanism family. The main chapter compresses that fact into a short open-front reading. This section restores the quantitative detail that supports that more austere conclusion.

\begin{table}[ht]
\centering
\caption{Escalon 2 phase summary. Phase-level reading of the front up to the opening of \texttt{S2-P3}.}
\label{tab:e2-phases}
\footnotesize
...
\texttt{S2-P3} is intentionally excluded from the tables below because the regime is already implemented and in execution, but does not yet have a stable comparative readout.
...
\texttt{a7}, \texttt{a10a}, \texttt{a10d} & various & concat & G10 & pilot mechanism sweep (concat) & retro./prov. \\
\texttt{*-pca} & various & FiLM/\texttt{pca} & G10 & pilot mechanism sweep (FiLM) & retro./prov. \\
\texttt{*-ab} & various & attn bias & G10 & pilot mechanism sweep (attn bias) & retro./prov. \\
...
\texttt{P3-*} & E2 descs & foundation speech & E2-P3 & encoder-regime comparison (WavLM) & planned \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
E1 canonical & MERT (${\sim}330$M) & MIDI Transf.\ (${\sim}13$M) & shared 256d & VICReg & impl. \\
...
E2 P3 & WavLM-Large frozen & EGG encoder & shared proj. & cross-modal obj. & opening \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Front} & \textbf{Positive unit} & \textbf{Hard negatives} & \textbf{Pool} & \textbf{Grouping} & \textbf{Score} \\
\midrule
...
Downstream extension & Gate 6 opens AMT as validation outside retrieval & Tested whether descriptor effects survive outside flagship task & active \\
...
Mechanism clarification & Gate 10 disentangles descriptor from injection route & Converts compressed retrospective band into causal question & active \\
New sensor-physics front & E2: Speech$\leftrightarrow$EGG with strong baseline, first null & Extends program outside symbolic music pairing & active \\
Encoder-regime comparison & S2-P3: WavLM-Large frozen on speech side & Shifts live question from mechanism to encoder regime & active \\
Synthetic-physical bridge & E3: Audio$\leftrightarrow$Lissajous & Creates first explicitly shared object with Beacon & horizon \\
Cardiovascular expansion & E4: ECG$\leftrightarrow$PPG & Extends logic beyond acoustics into physiology & horizon \\
\bottomrule
\end{tabularx}
\end{table}

This sequence matters because it shows what the current manuscript does and does not claim. The strong computational closure belongs to the first escalon. The second escalon has a serious baseline and a serious null. The later escalones are already architecturally meaningful, but they remain programmatic horizons rather than closed empirical chapters.
...
Shared object emergence & Phideus + Beacon & Lissajous as object generable, visible, and learnable & First direct shared experimental object & active \\
...
At the cut of this manuscript, the program is asymmetrical but coherent. Phideus has one strong closed computational front, one active new-domain front, one retrospective clarification branch, and two named horizons beyond the current evidence. Beacon has a stabilized device logic, active experimental lines, and a clear technological horizon. PMP plus Beacon has already become a repeated field practice whose mechanism is still being formalized. The manuscript is written exactly at that moment: after the probes have become real, but before their convergence has hardened into a single methodological or technological closure.
...
The conjectural core can then be stated in a disciplined way.
