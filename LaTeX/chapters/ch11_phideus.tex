...
From that paired data, Phideus builds a dual neural architecture. There is one encoder for each modality, because each signal has to be transduced into its own latent representation before comparison becomes possible. On top of those encoders sit projection heads that map both sides into a shared embedding space. The encoders vary by front: sometimes they are smaller Transformer or CNN-Transformer models trained from scratch for traceability; sometimes the audio side uses a local MERT-inspired encoder for the same reason; and sometimes a later comparison regime introduces a stronger frozen backbone such as MERT-330M or WavLM on one side of the pair. But the basic architecture remains a paired encoder system followed by learned projections into a common space.
...
Escalon 1 begins in music, with paired piano audio and piano MIDI from MAESTRO v3.0.0. This is the most hospitable opening because the musical event is held constant while the inscription changes. Audio records the acoustic result. MIDI records the performed gesture as symbolic event structure. The audio side is encoded with a local MERT-inspired encoder (\texttt{MERTEncoderLite}, roughly 60 million parameters) rather than with the full 330M HuggingFace backbone; the MIDI side uses a smaller Transformer trained from scratch; both project into a shared 256-dimensional space under VICReg \parencite{li_2024,bardes_2022}. The question is clean: when two inscriptions of the same event are already aligned at the dataset level, does descriptor guidance produce a stronger and more structured shared space than an unguided baseline?
...
Escalon~3 & Audio XY & Lissajous figure & Synthetic + Analog & Closed through P6 \\
...
% Audio side
\node[box, fill=blue!10] (ea) at (-3, 0) {Audio\\Encoder\\(MERTLite, ${\sim}60$M)};
\node[smallbox, fill=orange!15] (da) at (-5.1, -1.8) {Descriptor\\$d_a$};
...
\node[smallbox, fill=orange!15] (dm) at (5.1, -1.8) {Descriptor\\$d_b$};
...
\draw[arr] (xa) -- (ea);
\draw[arr] (ea) -- (pa);
\draw[arr] (da.south east) -- ([xshift=-2pt,yshift=2pt]pa.west);
\draw[arr] (pa) -- (za);
\draw[arr] (xm) -- (em);
\draw[arr] (em) -- (pm);
\draw[arr] (dm.south west) -- ([xshift=2pt,yshift=2pt]pm.east);
\draw[arr] (pm) -- (zm);
\draw[arr] (za) -- (loss);
\draw[arr] (zm) -- (loss);
...
The cleanest lesson of Escalon 1 is that route matters, but not in the simple sense that attention-based injection universally dominates concatenation. The canonical dual concat arm \texttt{d4a4} remains the strongest closed retrieval reference of the front, and the later retrospective mechanism sweep showed concat $>$ FiLM/\texttt{pca} $\gg$ attention bias within that branch. What the program has established more carefully is that descriptor signal can become highly productive when it is allowed to reorganize interaction rather than appear only as static side information pasted onto an embedding.
...
This distinction also stabilizes the reading of the chapter's own evidence. D4 and A4 answer the question of content. Concat, cross-attention, attention bias, reverse cross-attention, and projection conditioning answer the question of route. Arms such as d4a4, a4r, or d4-a4r belong to the level where both variables meet. Figure 11.2 schematizes the three most legible routes, while Appendix~C and that published companion paper~\parencite{fernandez_2026_phideus} carry the full implementation-level inventory. The later sections can then read results without collapsing these layers into one another.
...
\section[The central result: descriptor-guided cross-modal learning works]{The central result:\texorpdfstring{\\}{ }descriptor-guided cross-modal learning works}


The strongest closed result of Phideus remains the descriptor-guided validation of the canonical dual arm in Escalon 1. The d4a4 arm is supported by five independent training runs, yielding a mean S of 84.0 percent with a standard deviation of 2.7 percentage points (range 81.4--87.6). The unguided baseline D0, evaluated under the same five-seed training design, reaches 75.2 percent with a standard deviation of 2.3 percentage points. The gap of 8.8 percentage points is large (Cohen $d = 3.51$, $p < 0.001$) and stable across seeds: the observed five-seed ranges do not overlap, with the worst guided seed (81.4) exceeding the best unguided seed (77.4) by 4.0 percentage points. The result must be read against the explicit pipeline just defined: descriptor content, route of injection, projection into shared space, and only then retrieval.

That retrieval advantage is supported by three causal lines of evidence. The first is inference-time ablation. When the A4 signal is zeroed at evaluation while keeping the trained weights fixed, performance collapses from 83.8 percent to 7.8 percent. The descriptor is not acting as decorative side information. The model depends on it. The second is parameter-matched degradation. Random, zeroed, or temporally shuffled substitutes fall back to the unguided band, preserving an 8.0-to-9.8-percentage-point causal gap between the real descriptor and its degraded controls. The advantage therefore comes from structured information rather than from added capacity alone. The third is pre-registration. Prediction matrices and admissible inference rules were written before the main readout, constraining interpretation before the relevant numbers were seen. That control does not make post-hoc reasoning impossible, but it sharply limits the most common forms of confirmation drift \parencite{pearl_2009}.

The evidence does not stop at retrieval. Cross-encoder alignment, measured by CKA, improves across the guided arms, but not all in the same way. The largest gain appears in the best aligned arm, \texttt{d4-a4r}, which rises from 0.435 in D0 to 0.794, roughly an 82 percent increase \parencite{kornblith_2019}. The flagship dual arm \texttt{d4a4} rises to 0.659, roughly 51 percent above D0, while also delivering the best retrieval and the strongest pre-projection retention. At the same time, the descriptor-guided arms preserve roughly 29 percent more cross-modal information before the projection bottleneck. The descriptor is therefore not merely helping the model rank partners more effectively. It is altering the geometry through which both encoders are brought into relation.
...
\footnotesize
\begin{tabularx}{\textwidth}{@{}P{0.16\textwidth}P{0.19\textwidth}P{0.18\textwidth}Y@{}}
\toprule
Dimension & Test & Result & Interpretation \\
\midrule
Retrieval & Training-seed closure & $84.0\% \pm 2.7$pp (\texttt{d4a4}) & Flagship guided arm replicated across five independent training seeds \\
Causal (ablation) & Inference-time zeroing & $83.8\% \to 7.8\%$ & Descriptor causally necessary \\
Causal (controls) & Parameter-matched degradation & $+8.0$ to $+9.8$pp gap & Information, not parameters \\
Geometric & CKA alignment & $+82\%$ best arm / $+51\%$ \texttt{d4a4} & Representational reorganization across guided arms \\
...
Fine-grained & Hard negatives & $> 94\%$ across canonical arms & Strong temporal identity resolution across the front \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Canonical cross-modal retrieval reference across the main Escalon~1 arms ($n = 5$ independent training seeds per row).}
...
\midrule
d4a4 & 84.0\% & 2.7pp & 81.4--87.6 & $+8.8$pp & 3.51 & $p < 0.001$ \\
...
The next fronts have not all remained equally future-tense. Escalon~3 has already carried ratio into a domain where it is exactly specifiable, visible, and audible at once, creating the first experimental convergence with Beacon around Lissajous objects. Escalon~4 remains the future extension outside acoustics into cardiovascular physiology. Phideus can make harmonic hypotheses operational, but it cannot close the entire phenomenon from inside computation alone.


\section{Experimental implications of the activation conjecture}

Chapter~10 matters here because it sharpens what a computational probe should ask. If storage and retrieval are formally distinct, then a learned latent space should not be tested only with the same relational logic that helped stabilize it. It should also be tested with queries that challenge the space without simply collapsing into arbitrary noise. In practical terms, that means Phideus can compare integer-ratio probes, consonant neighborhoods, and $\phi$-offset or noble-number perturbations as different ways of asking what a learned manifold preserves, when it locks, and when it becomes newly readable.

The point is not to turn Phideus into a golden-ratio machine. The point is to give the computational branch a sharper family of contrasts. A ratio-guided descriptor may help organize storage-like regularity in the latent space. A $\phi$-offset query may help reveal whether that organization can be activated, traversed, or read without being overwritten by the same commensurate logic that produced the lock in the first place. That distinction creates a more exact experimental vocabulary for the fronts already named in this chapter, especially where the object is explicitly geometric or phase-bearing.

These implications remain probe-specific. They do not alter the central closed result of Escalon~1, which is still the validation of descriptor-guided cross-modal learning as a real mechanism. What they do is open a new layer of questions for the fronts that follow: whether some queries stabilize and others activate, whether latent geometry responds differently to commensurate and maximally non-locking perturbations, and whether the readout problem can be operationalized without being confused with storage itself. The next chapter takes up the complementary side of that distinction in physical space, through sustained acoustic fields, embodied response, and acoustic-physiological experience.
