...
The strongest closed result of Phideus remains the descriptor-guided validation of the canonical dual arm in Escalon 1. The d4a4 arm is currently supported by five evaluation-seed reads over a single trained checkpoint, yielding a reference mean S of 84.1 percent with a standard deviation of 2.3 percentage points. The unguided baseline D0, evaluated across five independent training seeds, reaches 75.2 percent with the same standard deviation. The gap is therefore large and stable enough to anchor the chapter's main claim, but the two rows should not be read as if they expressed the same kind of variance: d4a4 currently measures evaluator variance around a fixed trained model, whereas D0 measures full training variance across reruns. Even under that caveat, the guided arm remains well above the unguided baseline, and the result must be read against the explicit pipeline just defined: descriptor content, route of injection, projection into shared space, and only then retrieval.
...
\midrule
Retrieval & Evaluation-seed reference & $84.1\% \pm 2.3$pp (\texttt{d4a4}) & Strong guided reference; full training-seed replication pending \\
...
\begin{table}[ht]
\centering
\caption{Canonical cross-modal retrieval reference across the main Escalon~1 arms ($n = 5$ per row; \texttt{d4a4} uses evaluation-seed reads over a fixed checkpoint).}
...
\midrule
d4a4 & 84.1\% & 2.3pp & 82.6--88.4 & $+8.9$pp & pending & pending \\
